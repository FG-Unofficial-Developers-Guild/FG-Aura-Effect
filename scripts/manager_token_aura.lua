...
function getTokensWithinShape(bPoint, nShape, tokenSource, ngParamA, ngParamBoptional, ngParamCoptional, ngParamDoptional, ngParamEoptional)
	...
	-- find tokens within max spherical search distance then determine if center point is within ORTHOGONAL
	--(not tilted) 3D shape (sphere, cyclinder, 3Drectangle) based on parameters provided
...
function isPointInOrientedRect3D(pointX, pointY, pointZ, cX, cY, length, width, orientation, z_min, z_max)
	...
	local halfL = l / 2
	local halfW = w / 2
	if lx < cx - halfL or lx > cx + halfL then
		return false
	end
	if ly < cy - halfW or ly > cy + halfW then
		return false
	end

	-- Step 3: Check Z bounds
	if z < z_min or z > z_max then
		return false
	end
